    ...
    inout ingress_intrinsic_metadata_for_tm_t        ig_tm_md)
{
    ...
    RegisterAction<bit<16>, bit<16>, bit<32>>(layer_16bit) insert_16bit=
    {
        ...
        {
            if (register_data != 65535 && register_data < (bit<16>)meta.mini)
            result = (bit<32>) register_data;

            if (register_data < (bit<16>)meta.mini)
            register_data = register_data |+| 1;
        }
    };
    ...
    RegisterAction<bit<8>, bit<16>, bit<32>>(layer_8bit) insert_8bit=
    {
        ...
        {   
            if (register_data != 255 && register_data < (bit<8>)meta.mini)
            result = (bit<32>)register_data;
    ...
    apply
    {
        ...
        meta.mini = insert_8bit.execute(meta.index_8);
        meta.mini = insert_16bit.execute(meta.index_16);
        meta.mini = insert_32bit.execute(meta.index_32);
