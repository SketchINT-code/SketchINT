...
void distribution_test(TRACE *traces, FLOW_ITEM *items, int item_cnt, int memory, int sketch_id, int opt, FILE *output_file)
{
    ...
    unordered_map<int, double> ni_dist;
    ...
    for (int rep = 0; rep < REP_TIME; rep++)
    {
        ...
        for (int i = 0; i < item_cnt; i++){
            ...
            int val_ni = sketch->query(items[i].key.key, TUPLE_LEN, 0);
            ni_dist[val_ni]++;
            
        }

        double zi_ni = 0, mu_ni = 0;

        for (int i=1; i<=max_f; i++){
            zi_ni += fabs(real_dist[i] - ni_dist[i]);   //fen zi

            mu_ni += (real_dist[i] + ni_dist[i]) / 2.0;   //fen mu
        }

        double WMRE = zi_ni/mu_ni;
