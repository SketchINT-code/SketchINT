...
public:
    int tot_packets;
    TowerSketch() {}
    TowerSketch(vector<uint32_t> &w, uint32_t h, vector<uint8_t> &counter_len, int _opt, int _threshold = 0)
    {
    ...
    void query_distribution(double *results, int level)
    {
...
public:
    TowerSketchCU() {}
    TowerSketchCU(vector<uint32_t> &w, uint32_t h, vector<uint8_t> &cs, int _opt, int _threshold = 0) : TowerSketch(w, h, cs, _opt, _threshold) {}
...
class TowerSketchHalfCU0 : public TowerSketch
{
public:
    TowerSketchHalfCU0() {}
    TowerSketchHalfCU0(vector<uint32_t> &w, uint32_t h, vector<uint8_t> &cs, int _opt, int _threshold = 0) : TowerSketch(w, h, cs, _opt, _threshold) {}
    ...
    virtual void insert(const char *key, uint16_t key_len, uint32_t row_id, uint32_t f = 1)
    {
        ...
        for (uint8_t i_level = 0; i_level < level; ++i_level)
        {
            ...
            if (min_val != UINT32_MAX && ret_val[i_level] < min_val + f)
                mat[i_level].insert_with_hash_value(key, key_len, row_id, hash_value[i_level], min_val + f - ret_val[i_level]);
        }
        if (opt)
        {
            task_record(key, key_len, row_id, pre_freq);
...
class TowerSketchHalfCU1 : public TowerSketch
{
public:
    TowerSketchHalfCU1() {}
    TowerSketchHalfCU1(vector<uint32_t> &w, uint32_t h, vector<uint8_t> &cs, int _opt, int _threshold = 0) : TowerSketch(w, h, cs, _opt, _threshold) {}
    ...
    virtual void insert(const char *key, uint16_t key_len, uint32_t row_id, uint32_t f = 1)
    {
        ...
        for (int i_level = level - 1; i_level >= 0; --i_level)
        {
            ret_val[i_level] = mat[i_level].query_with_hash_value(key, key_len, row_id, hash_value[i_level]);
            if (ret_val[i_level] < mat[i_level].counter_max)
                min_val = min(min_val, ret_val[i_level]);
