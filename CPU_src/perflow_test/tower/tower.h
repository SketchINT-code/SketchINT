...
public:
	...
	void init(uint32_t in_w, uint32_t in_h, uint8_t in_counter_len, uint32_t random_seed)
	{
		...
		if (counter_len == 32) mask = counter_max = 0xffffffff;
		else mask = counter_max = (1u << counter_len) - 1;
...
public:
	...
	void task_record(const char* key, uint16_t key_len, uint32_t row_id, int pre_freq){
		if (opt == 1){ // heavy_hitter or heavy_change
	...
	void query_distribution(double* results, int level){
		...
		em_fsd_algo->set_counters(mat[level].w, (uint32_t*)counters);
