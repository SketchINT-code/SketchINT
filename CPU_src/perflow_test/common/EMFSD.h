...
private:
    ...
    struct BetaGenerator
    {
        ...
        int now_flow_num;
        int flow_num_limit;
        vector<uint32_t> now_result;

        explicit BetaGenerator(uint32_t _sum, uint32_t _inf) : sum(_sum), inf(_inf)
        {
            now_flow_num = 0;
            //            if (sum > 2000) {
            //                flow_num_limit = 2;
        ...
        bool get_new_comb()
        {
            for (int j = now_flow_num - 2; j >= 0; --j)
            {
                ...
                if (sum < partial_sum)
                    continue;
                uint32_t remain = sum == inf ? inf : sum - partial_sum;
    ...
    double get_p_from_beta(BetaGenerator &bt, double lambda, vector<double> &now_dist, double now_n)
    {
        ...
        for (auto &kv : mp)
        {
            ...
            ret *= (std::pow(lambda_i, fi)) / factors[fi];
        }
...
public:
    ...
    void set_counters(uint32_t _w, uint32_t *counters, uint32_t _inf)
    {
        inited = true;
        w = _w;
        inf = _inf;
    ...
    void next_epoch()
    {
        ...
        for (uint32_t i = 1; i < counter_dist.size(); ++i)
        {
            ...
            BetaGenerator bts1(i, inf), bts2(i, inf);
            double sum_p = 0;
            while (bts1.get_next())
            {
